...
def Matrix_MSE(list_matrices, h, lmbd):
    ...
    biais = nsample * np.power(means - V_,2)
    X_2 = np.array([sum(np.power(x,2)) for x in zip(*list_matrices)])
    #variance = np.array([sum(np.power(x,2)) / niter - np.power(means,2) for x in zip(*list_matrices)])
    variance = nsample * np.array(X_2 / niter - np.power(means,2))
    MSE = np.power(biais + variance,1)
    return(MSE)

def plot3d(Matrix,h, lmbd,save = False, Matrix_2 = None):
    """
        Plot a matrix
    ...
    ax.set_zlabel(r'$MSE$')
    if save :
        plt.savefig('/home/aboulin/Documents/stage/var_FMado/max_stable/output/image_3d.pdf')
    else :
        return fig, ax
...
sigma = np.sqrt(5)
h = np.linspace(0,20.0,100)
lmbd = np.linspace(0.0,1,100)
niter = 300
nsample = 105
#results = simgen(h,lmbd, niter, nsample, sigma)


#with open("data_sigma_5_NN_1024.txt", "wb") as data:
