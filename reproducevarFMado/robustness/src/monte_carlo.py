...
class Monte_Carlo(object):
    ...
    def huber_contamination(self, inv_cdf_s, show_index = False, unimargin = False):
        """
        ...
        X[~mask,:] = self.copula_sane.sample_unimargin()
    ...
    def adversarial_contamination(self, inv_cdf_s, show_index = False, unimargin = False):
        """
        ...
        X = self.copula_sane.sample_unimargin()
        index = np.argsort(np.linalg.norm(X-np.array([0.5,0.5]), axis = 1))
        X = X[index]
